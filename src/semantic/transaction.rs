...
impl<'a> SemanticTransaction<'a> {
    ...
    pub async fn select_outbox_messages(
        &self,
        limit: u64,
        ...
        let mut rows = self
            .conn
            .query(
                "SELECT outbox_id, record_kind, object_id, payload, created_at_ns
                 FROM outbox
        ...
        let mut result = Vec::new();
        while let Some(row) = rows.next().await? {
            result.push(outbox::Row::from_sql_row(&row)?.into_shared_message()?);
        }
        Ok(result)
    }
    ...
    pub async fn trim_outbox(
        ...
        self.conn
            .execute("DELETE FROM outbox WHERE outbox_id <= ?1", (end,))
            .await?;
        Ok(())
    }

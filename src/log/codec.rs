...
fn encode_common_headers(msg: &SharedMessage) -> eyre::Result<Vec<Header>> {
    let mut headers = vec![];
    ...
    match msg {
        ...
        SharedMessage::TextObjectUpdate {
            ...
            headers.push(Header::new("object_id_b64", object_id.encode_b64()));
        }
        SharedMessage::BinaryObjectPut {
            ...
            headers.push(Header::new("object_id_b64", object_id.encode_b64()));
            headers.push(Header::new("writer_id_b64", writer_id.encode_b64()));
...
fn decode_b64_bytes(header_name: &str, value: Option<&str>) -> eyre::Result<Bytes> {
    let value = value.ok_or_else(|| eyre::eyre!("log record missing {header_name} header"))?;
    let bytes = base64::engine::general_purpose::STANDARD
        .decode(value)
        .map_err(|_| eyre::eyre!("invalid log record {header_name} header"))?;
    Ok(Bytes::from(bytes))
}

fn decode_object_id(headers: &[Header]) -> eyre::Result<ObjectId> {
    ObjectId::decode_b64(
        header_str(headers, "object_id_b64")?
            .ok_or_else(|| eyre::eyre!("log record missing object_id_b64 header"))?,
    )
    .ok_or_else(|| eyre::eyre!("invalid log record object_id_b64 header"))
}
...
pub fn s2_payload_to_shared_message(
    ...
    let msg = match kind {
        ...
        "binary_put" => SharedMessage::BinaryObjectPut {
            object_id: decode_object_id(headers)?,
            writer_id: DaemonWriterId(decode_b64_bytes(
                "writer_id_b64",
                header_str(headers, "writer_id_b64")?,
            )?),
...
pub fn shared_to_s2_package(msg: SharedMessage) -> eyre::Result<S2Package> {
    let payload = msg.payload().clone();
    let payload_size = payload.len();
    let pointer_headers = encode_common_headers(&msg)?;

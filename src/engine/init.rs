...
pub async fn run(config: InitConfig) -> eyre::Result<InitResult> {
    ...
    let imported_actions = match next_work {
        ...
        NextWork::Import(started) => {
            ...
            match await_commit_import_epoch(&mut semantic).await? {
                NextWork::None => {}
                NextWork::Import(_) => {
                    eyre::bail!("init import epoch returned a second import epoch");
                }
                NextWork::Project(_) => {
                    eyre::bail!("init import epoch returned projection work");
                }
            }
...
async fn await_commit_import_epoch(semantic: &mut SemanticClient) -> eyre::Result<NextWork> {
    match semantic
        .next()
        .await

...
impl NamespaceDoc {
    ...
    pub fn add_new_object(&self, object_id: &ObjectId, kind: ObjectKind, creator: &DaemonWriterId) {
        ...
        let nested = MapPrelim::from([
            (Self::FIELD_KIND.to_string(), Any::from(kind)),
            (
                Self::FIELD_CREATOR.to_string(),
                Any::from(creator.0.to_vec()),
            ),
        ]);
        self.objects.insert(&mut txn, object_key.clone(), nested);
    ...
    pub fn add_new_claim(
        ...
        let nested = MapPrelim::from([
            (
                Self::FIELD_OBJECT_ID.to_string(),
                Any::from(object_id.0.to_vec()),
            ),
            (Self::FIELD_PATH.to_string(), Any::from(path.to_db_path())),
        ]);
        self.claims.insert(&mut txn, claim_key.clone(), nested);
...
static CONVENIENCE_CLIENT_ID_COUNTER: AtomicU64 = AtomicU64::new(20_000_000);
...
pub fn apply_namespace_update(base_state: &[u8], update_bytes: &[u8]) -> Result<Bytes> {
    let doc = NamespaceDoc::from_full_state(fresh_convenience_client_id(), base_state)?;
    doc.apply_update(update_bytes)?;
    Ok(doc.encode_full_state())
}

pub fn encode_state_delta(base_state: &[u8], next_state: &[u8]) -> Result<Bytes> {
    let base_doc = NamespaceDoc::from_full_state(fresh_convenience_client_id(), base_state)?;
    let next_doc = NamespaceDoc::from_full_state(fresh_convenience_client_id(), next_state)?;
